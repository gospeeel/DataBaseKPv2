...
{
	printf("---------------------------------------------------------------------------------------------------------\n");
	printf("|| Название производителя  ||  Форм-фактор ||  Типоразмер ||  Высота  ||  Цвет  ||  Наличие подсветки  ||\n");
	printf("---------------------------------------------------------------------------------------------------------\n");
	for (int b = 0; b < shet; b++)
	{
		printf("||      %s              ||      %s      ||       %s       ||   %dсм   ||   %s   ||   %s   ||\n", comp[b].manufacturer, comp[b].formfactor, comp[b].typesize, comp[b].height, comp[b].color, comp[b].availability);
		printf("---------------------------------------------------------------------------------------------------------\n");
	}
	system("pause");
	return 0;
...
void displayN2(PC* comp, int shet)
{
	printf("||%s|%s||%s||%dсм||%s||%s||\n", (comp + shet)->manufacturer, (comp + shet)->formfactor, (comp + shet)->typesize, (comp + shet)->height, (comp + shet)->color, (comp + shet)->availability);
	printf("---------------------------------------------------------------------------------------------------------------------------\n");
}
...
int vstavit(char* nazvaniefaila, PC* comp, int shet)
{
	...
	{
		fprintf(data, "---------------------------------------------------------------------------------------------------------\n");
		fprintf(data, "|| Название производителя  ||  Форм-фактор ||  Типоразмер ||  Высота  ||  Цвет  ||  Наличие подсветки  ||\n");
		fprintf(data, "---------------------------------------------------------------------------------------------------------\n");
		for (c = 0; c < shet; c++)
		{
			fprintf(data, "||%s||%s|%s||%d||%s||%s||\n", comp[c].manufacturer, comp[c].formfactor, comp[c].typesize, comp[c].height, comp[c].color, comp[c].availability);
			fprintf(data, "---------------------------------------------------------------------------------------------------------\n");
		}
		fclose(data);
